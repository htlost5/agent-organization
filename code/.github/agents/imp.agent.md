...
- バグ修正・デバッグ
- 実装ログの出力
- `logs/impl/implementation/` への実装・デバッグログ出力
...
- ARC からのインターフェース仕様書（`shared/impl/specs/interfaces/IF-XXX.md`）
- ARC からのアーキテクチャ決定（`shared/impl/decisions/architecture/ADR-XXX.md`）
- 既存コードベース

## Outputs

- 実装コード（変更ファイル）
- Implementation Log: `logs/impl/implementation/YYYY-MM-DD_IMP_{topic}.md`
...
- 実装ログは `logs/impl/implementation/YYYY-MM-DD_IMP_{topic}.md` の命名規則に従う
